...
| `index.html` | 课程选择页 |
| `<slug>/` | 单门课（`course.json` 含大纲/术语/测验、`chapters/*.html`，工作流 A/B 生成） |

## 启动

在**仓库根目录**启动（内置进度写入 API；勿只 serve 子目录，否则 `fetch('courses.json')` 失败）：

```powershell
Set-Location d:\project\study-llm-dev-all
node scripts/serve-courses.mjs
# http://localhost:3000/ → 课程中心；单课 http://localhost:3000/<slug>/
```

每门课目录含 **`progress.local.json`**（bootstrap/assemble 创建），学习时自动更新；可 gitignore，勿提交个人进度。
...
**工作流 B**：写 `chapters/<id>.html`，同次更新 `course.json` 的 `terms` / `quizzes`（勿用 `manifest-*.json`）→ `assemble` 生成 `index.html`（勿手改正文壳）。
