...
3. 已跑 `/course-gate`（含 `review-chapter.mjs` + `review-demo.mjs`）且无 blocking error；否则只输出结构/量化修复项
4. 按需读 `courses/<slug>/course.json`、课程 `demos/`
5. 量化数值 **仅以** `<skill-root>/config/chapter-quality.json` 为准（**勿**在 command / Agent prompt 中写死行数、题数等）
